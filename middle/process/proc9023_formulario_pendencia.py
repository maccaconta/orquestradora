...
class Proc9023FormularioPendencia(Consumers):
    async def get_formulario_pendencia(request_) -> dict:
        """
        ...
        context_ = await Consumers.serv_formulario_pendencia(params_, request_["service"])
        ...
        if context_ == {}:
            ...
            try:
                ...
                context_tratado_ = splitDict("proc9023_", context_)
                try:
                    cod_ = int(context_tratado_['proc9023_codigo'])
                    if cod_ == 0:
                        return_= {
                            'text': 'api_ok',
